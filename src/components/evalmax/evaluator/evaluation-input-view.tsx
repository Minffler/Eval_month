...
export default function EvaluationInputView({ 
  ...
  console.log('=== EvaluationInputView Debug ===');
  ...
  // myEmployees가 변경될 때 localEmployees 업데이트 (초기 로드 시에만)
  React.useEffect(() => {
    // myEmployees와 localEmployees가 다를 때만 업데이트
    const hasChanges = JSON.stringify(myEmployees) !== JSON.stringify(localEmployees);
    if (hasChanges && myEmployees.length > 0) {
      console.log('=== EvaluationInputView - Updating localEmployees from myEmployees ===');
          ...
          {Object.keys(gradingScale || {}).length > 0 ? (
            <Select 
              ...
              onValueChange={(g) => {
                ...
                console.log('==================================');
                onGradeChange(employee.id, g === 'none' ? null : g as Grade);
              }}
            >
